...
public class AIPath : MonoBehaviour {
	...
	/// <summary>
	/// This is the patrol Logic.
	/// Aristotelis Triantafyllidis
	/// </summary>
	public virtual void OnTargetReached () {
		if (Vector3.Distance (target.position, endPos.position) <= 0.2f) {
			target = startPos;
		} else {
			target = endPos;
		}
	...
	public virtual void Update () {
		...
		if (!following) {
			...
			Transform newTrans = GameObject.FindGameObjectWithTag ("Player").transform;

			if (Time.time - lastRepath > repathRate && seeker.IsDone ()) {
				lastRepath = Time.time + Random.value * repathRate * 0.5f;

				seeker.StartPath (transform.position, newTrans.position);
			} 
